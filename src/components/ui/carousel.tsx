...
const Slide = ({ slide, index, current, handleSlideClick }: SlideProps) => {
  ...
  const handleNavigation = () => {
    router.push(`/portfolio/detail/${slide.id}`); // Navigate to portfolio details page
  };
  useEffect(() => {
    const animate = () => {
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      ...
      >
        <div
          className="absolute top-0 left-0 w-full h-full bg-primary-foreground rounded-[1%] overflow-hidden transition-all duration-150 ease-out"
          style={{
            transform:
              current === index
